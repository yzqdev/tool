 ...
 * @param inputPath
 */
export async function genReadme(inputPath: string) {
  //1.接受命令行命令
  //3.判断路径是否存在
  //2.修正路径
  //[2]是输入的路径名
  ...
  async function genReadmeFiles(filePath: string) {
    ...
    if (state.isFile()) {
      ...
      if (existsSync(path.resolve(filePath, "README.md"))) {
        ...
        let content = "# " + filePath.split(path.sep).pop();
        await writeFile(
...
export async function genRecurseReadme(inputPath: string) {
  async function readFileList(dir: string, filesList: SingleDirPath[] = []) {
    ...
    let dirPath: SingleDirPath = {
      name: path.resolve(dir).split(path.sep).pop()!,
      files: [],
    };
  ...
  let md = "";
  let head = path.resolve().split(path.sep).pop();
...
export async function genSingleReadme(inputPath: string) {
      ...
      if (
        ...
        if (pattern.test(data.toString())) {
          dirPath.files.push({
            ...
            filename: fullPath.split(path.sep).join("/"),
          });
        } else {
          console.log(fullPath);
          dirPath.files.push({
...
export async function getAllMarkdowns(inputPath: any) {
  async function readFileList(dir: string, filesList: string[] = []) {
    const files = await readdir(dir);
    let dirPath = { name: path.resolve().split(path.sep).pop(), files: [] };
    for (const item of files) {
      let fullPath = path.join(dir, item);
      const fileStat = await stat(fullPath);
...
export async function genMarkdownImgs(file: string) {
  ...
  for (let item of imgs) {
    try {
      ...
      console.error(pc.red(`下载图片失败: ${item}`), e);
    }
  }
  ...
  async function downloadImage(
    ...
    await writeFile(path.join(folder, fileName), response.data);
    return `![${replacerFileName(url)}](./${imgFolder}/${fileName})`;
  }
}
